...
{
    public static JsonObject GetLanguageData(string folder, string basename, string language = "")
        => GetLanguageData(GetLanguageFile(folder, basename, language));
    ...
    public static IEnumerable<string> GetBaseLanguageFileNames(string folder, string language)
    {
        return Directory.EnumerateFiles(folder, "*", SearchOption.AllDirectories)
            ...
            .GroupBy(sub => sub)
            .Where(sub => File.Exists(GetLanguageFile(folder, sub.Key, language)))
